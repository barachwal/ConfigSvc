...
template <typename T> T ConfigSvc::GetValue(const std::string& module, const std::string& unit) const {
    if (IsRegistered(module)) {
        if(m_config_modules.at(module)->Config()->IsDefined(unit)) {
            return std::any_cast<T>(m_config_modules.at(module)->Config()->GetValue(unit));
        } else {
            throw std::invalid_argument("ConfigSvc::GetValue:: "
                                        "Module( " + module + " ): Given unit (" + unit + ") is not defined.");
        }
    } else {
